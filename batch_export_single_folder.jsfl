...
var volumeName = folderItems[0].replace("|", "");
// On the Mac we need to remove the first item in the path
var saveDir = volumeName+":/" + folderItems.slice(1,folderItems.length).join("/") + "/";
fl.trace( folderItems[0]);
fl.trace(saveDir);
...
if (saveDir) {
    ...
    for (idx in doc.library.items) {
        ...
        //if(currentItem.linkageIdentifier != undefined) {
			...
			var tempFolder = saveDir+currentItem.name;
			tempFolderString = tempFolder.slice(0, tempFolder.lastIndexOf("/"));
			if(FLfile.exists("file:///"+tempFolderString)) {
				fl.trace(currentItem.itemType);
				//continue;
				exportItemAsPng(currentItem, 2.0)
			} else {
				fl.trace(tempFolderString+" not existed.")
				continue;
			}
			
		} else {
			// check and create folder.
			var wholeFolder = saveDir+currentItem.name;
			
			FLfile.createFolder("file:///"+wholeFolder);
		}
    }
}
...
function exportItemAsPng(item, scaleFactor) {
	...
	//return;
    var pngName = saveDir + item.name +".png";
    ...
    var w = doc.selection[0].width*scaleFactor;
    var h = doc.selection[0].height*scaleFactor;
 
    // create a publishing profile for this output with the filename, width and height set
	 fl.trace(saveDir);
	//return;
    createProfile(item.name, w, h, saveDir);
    ...
    exportdoc.selectAll();
	exportdoc.scaleSelection(scaleFactor, scaleFactor, "center");
